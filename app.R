...
ui <- navbarPage(
    ...
    tabPanel(
# Add CSS to give panels floating over map, a style
...
#    })
    ...
    observeEvent(input$map1_shape_mouseover, label="event4e", {
        if (input$select_map_level == 'Municipality'){
            gis$id <- gis$shp@data$MAP_TITLE[gis$shp@data$ID == gis$mouseover_id]
        }
    })
    ...
    observe(if(!is.null(gis$mouseover_id)){ 
        label="event12d"
        gis$slice1 <- gis$shp@data$PROVINCE[gis$shp@data$ID == gis$mouseover_id]
    })
...
#    })
    ...
    observeEvent(input$map1_topojson_mouseover, label="event13a",{
     ...
     observe(if((!is.null(gis$mouseover_id) & input$select_map_level == 'Municipality')){ 
         label="event13b"
#         gis$slice2 <- town_tj_spd@data$MAP_TITLE[town_tj_spd@data$ID == gis$mouseover_id]
         gis$slice2 <- gis$shp@data$MAP_TITLE[gis$shp@data$ID == gis$mouseover_id]
    ...
    observeEvent(gis$mouseover_id,label="event22", {
        if (input$enable_hover == TRUE & input$select_map_level == 'Ward') {
            proxy <- leafletProxy(
                "map1", data = subset(
                    gis$shp, 
                    gis$shp@data$ID == gis$mouseover_id
                )
            )
            proxy %>%
                clearGroup('single') %>%
    ...
    observeEvent(input$map1_topojson_mouseover,label="event23", {
        if (input$enable_hover == TRUE & input$select_map_level == 'Municipality') {
            proxy <- leafletProxy(
                "map1", data = subset(
                    gis$shp,
                    gis$shp@data$MAP_TITLE == gis$slice2
                )
            )
            proxy %>%
                clearGroup('single') %>%
                ...
                div(
                    strong(
                        sub(
                            ...
                            replacement="",
                            x=gis$shp@data$MUNICNAME[gis$shp@data$ID == gis$mouseover_id])),
                    br(),
                    "Ward",
                    gis$shp@data$WARDNO[gis$shp@data$ID == gis$mouseover_id],
                    br(),
                    span(round(gis$shp@data$DENSITY[gis$shp@data$ID == gis$mouseover_id],1), HTML("people/km<sup>2</sup>"))
                )
            )
